...
function formatCurrency(amount: number, currency = "MXN", changeType = 1) {
  const isUsd = currency.toUpperCase() === "USD";

  // Si changeType < 1 (ej. 0.059), multiplicamos MXN * changeType para obtener USD
  // Si no se requiere conversión previa, el valor pasa directo
  const finalAmount = isUsd && changeType !== 1 ? amount / changeType : amount;
...
export async function POST(req: Request) {
  try {
    const { orderId, items, total, customer, currency, exchangeRate } = await req.json();

    const itemsHtml = items.map((item: any) => `
      <tr>
        <td style="padding: 10px 0; border-bottom: 1px dashed #ddd;">
          <div style="font-weight: bold; color: #102f67;">${item.product.name}</div>
          <div style="font-size: 12px; color: #666;">Cant: ${item.quantity} x ${formatCurrency(item.product.price, currency, exchangeRate)}</div>
        </td>
        <td style="padding: 10px 0; border-bottom: 1px dashed #ddd; text-align: right; font-weight: bold; color: #102f67;">
          ${formatCurrency(item.product.price * item.quantity, currency, exchangeRate)}
        </td>
      </tr>
    `).join('');
      ...
      <div style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 450px; margin: auto; border: 1px solid #eee; padding: 20px; color: #333;">
        ...
        <table style="width: 100%; margin-bottom: 20px;">
          <tr>
            ...
            <td style="text-align: right; font-size: 20px; font-weight: bold; color: #102f67;">${formatCurrency(total, currency, 1)}</td>
          </tr>
        </table>
